...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("too few or too many values given")
        def helper(formals, vals):
            if formals is not nil:
               child.define(formals.first, vals.first)
               helper(formals.second, vals.second)
        helper(formals, vals)
        # END Question 10
        return child
...
def do_if_form(expressions, env):
    ...
    elif scheme_false(condition) and len(expressions) == 3:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_true(test):
            # BEGIN Question 15A
            if clause.second is nil and test is True:
                return True
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)
    #print('formals:', formals);print('vals:', vals)
...
def check_formals(formals):
    ...
    while formals is not nil:
        if scheme_symbolp(formals.first) is False or formals.first in symbols:
            raise SchemeError
        symbols += formals.first
