...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while location is not hive:
            if (self.min_range <= distance <= self.max_range) and location.bees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 9
        location = self.place
        if self is self.queens[0]:
            ThrowerAnt.action(self, colony)
            while location is not None:
                if location.ant:
                    if location.ant not in self.doubled:
                        location.ant.damage = location.ant.damage * 2
                        self.doubled.append(location.ant)
                    if location.ant.container and location.ant.ant and location.ant.ant not in self.doubled:
                        location.ant.ant.damage = location.ant.ant.damage * 2
                        self.doubled.append(location.ant.ant)
