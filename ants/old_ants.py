...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.name == "Queen" and insect == insect.queens[0]:
                return None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 3B
        location = self.place
        #set starting range location
        min_range_count = 0
        while self.min_range > min_range_count:
            location = location.entrance
            if location == hive:
                return None
            min_range_count += 1
        #look for bees till max range is reached
        max_range_count = min_range_count - 1
        while max_range_count != self.max_range and location != hive:
            if not location.bees:
                location = location.entrance
                max_range_count += 1
            else:
                return random_or_none(location.bees)
        #if no bees are out of hive:
        return None
        # END Problem 3B
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        beesinplace = list(self.place.bees)
        for bee in beesinplace:
            Insect.reduce_armor(bee, self.damage)
        # END Problem 6A
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 9
        #check for impostor queen
        if self in self.queens[1:]:
        ...
        while location is not None:
            if location.ant:
                ...
                if location.ant.container and location.ant.ant and location.ant.ant not in self.doubled_ants:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        Insect.reduce_armor(self, amount)
        if self.armor <= 0 and self == self.queens[0]:
            bees_win()
        # END Problem 9
...
def make_stun(action):
    ...
    def stun(colony):
        return None
    return stun #can't return None so had to put return none into a function
    # END Problem EC

def apply_effect(effect, bee, duration):
    """Apply a status effect to a Bee that lasts for duration turns."""
    # BEGIN Problem EC
    #assign normal action to something else
    bee_action = bee.action

    def application(colony):
        nonlocal duration
        if duration > 0:
    ...
    #give bee the new action
    bee.action = application
    # END Problem EC
